...
- **Identidade visual:** `marca/design-guide.md` — LER ANTES de criar qualquer visual
...
**Arco (5 a 10 slides) — não pular etapas:**
1. **CAPA (hook)** — para o scroll (ver checklist de copy abaixo)
2. **Desenvolvimento** — 2–3 layouts entre `SOLO` / `DUO` / `NÚMERO` / `CITAÇÃO`; uma ideia por slide
3. **Reflexão** — penúltimo slide de conteúdo: o que isso muda na prática do leitor (não mais um dado solto)
4. **CTA FINAL** — ação específica ligada ao conteúdo (não “nos siga”)

Antes de criar HTML: ler `marca/design-guide.md`. Se estiver em branco, usar o "Estilo visual base" acima como default.

### Checklist de copy (antes do visual)

Aplicar **sempre** no texto. Se falhar, reescrever — não “melhorar no design”.

| Gate | Passa se… | Falha se… |
|------|-----------|-----------|
| **Scroll-stop** | Capa = tensão, dado concreto, contraste ou pergunta que o feed não ignora | “Hoje vamos falar…”, “X dicas de…”, meta-comentário, só logo/nome |
| **Densidade** | 1 ideia por slide; ≤ ~5 linhas visíveis; sem parágrafo de LinkedIn | Bullet de deck, 2+ ideias, bloco denso |
| **Arco** | Hook → desenvolve → reflexão → CTA | Só lista de fatos → CTA |
| **CTA** | Ação específica e ligada ao tema | “Nos siga”, “link na bio”, “salve” sem motivo |
| **Dados** | Número com fonte **ou** claramente opinião/experiência | Estatística inventada / “todo mundo sabe” |

Capa: título **máx ~8 palavras**. Oferecer **3 opções** de capa no checkpoint de texto.
...
Se o usuário não souber qual foi a última, perguntar (ou default: CAPA escura / destaque da marca).

### Linguagem (regra crítica)

Seguir `_memoria/preferencias.md`. Frases naturais, sem jargão de marketing, sem corporativês. O público real raramente fala "ticket médio", "performance", "B2B". Falar como ele fala.

Frase genérica que serve pra **qualquer** marca/tema → reescrever.
...
2. Ler `marca/design-guide.md` pra cores, fontes e logo
3. Identificar o tipo de conteúdo (1, 2 ou 3)
4. Definir o tema e o ângulo

### Passo 2 — Texto

Escrever o conteúdo seguindo tom + **checklist de copy**.

**Pra carrossel (5-10 slides):**
- Capa: 3 opções que passam scroll-stop
- Internos: 1 insight/slide, ≤ ~5 linhas, sem bullet de deck
- Penúltimo: reflexão prática (“o que isso muda pra você”)
- Final: CTA específico + logo

**Pra post único:**
- Frase principal = scroll-stop
- Apoio curto se precisar
- CTA sutil e específico

**CHECKPOINT:** Mostrar o texto completo **e** marcar mentalmente os 5 gates (hook, densidade, arco, CTA, dados). Esperar aprovação antes do visual.
...
1. Criar **um único `carrossel.html`** com TODOS os slides como `<div class="slide">` dentro do mesmo arquivo. Inline CSS, Google Fonts como única dependência externa. Aplicar:
   - Cores e tipografia de `marca/design-guide.md`
...
- Sempre ler `marca/design-guide.md` antes de criar qualquer visual
...
- Copy: checklist (scroll-stop, densidade, arco com reflexão, CTA específico, dados honestos) — design não salva texto fraco
