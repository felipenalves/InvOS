...
- **Voz canônica:** `marca/marca.md` + `pipeline/data/tone-of-voice.md` (marca vence).  
- **Oferta/ICP:** `memoria/empresa.md` — amarre CTA à oferta real do prestador.  
- **Output:** uma versão polida (ângulo único escolhido).  
- **Slides:** 7–9 (hook + conteúdo + CTA).  
- **Não inventar** estatística sem fonte; preferir prova do negócio se existir em `clientes/` ou histórico.

## Voice Guidance

Seguir exemplos de “pode dizer” em `marca/marca.md`.  
Método de carrossel (dado → contexto → tensão → CTA): ver `pipeline/data/tone-of-voice.md`.
>
